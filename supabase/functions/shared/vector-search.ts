...
function exactBoost(question: string, chunkText: string) {
  ...
  for (const term of EXACT_BOOST_TERMS) {
    if (q.includes(term) && c.includes(term)) boost += 0.05;
  }
  const quoted = q.match(/"([^"]+)"/g)?.map((item) => item.replaceAll('"', "")) ?? [];
  for (const phrase of quoted) {
    if (phrase.length > 2 && c.includes(phrase)) boost += 0.1;
  }
...
export async function retrieveKnowledgeChunks(
  ...
  const matchThreshold = options.matchThreshold ?? Number(Deno.env.get("RAG_MATCH_THRESHOLD") ?? DEFAULT_MATCH_THRESHOLD);
  ...
  const ranked = ((data ?? []) as RetrievedChunk[])
    .map((chunk) => ({
      ...
      combined_score: (chunk.combined_score ?? chunk.similarity ?? 0) + exactBoost(question, chunk.chunk_text),
    }))
  ...
  return {
    queryEmbedding,
    chunks: ranked,
    retrievalScore: ranked.length > 0
      ? ranked.reduce((sum, item) => sum + (item.combined_score ?? item.similarity ?? 0), 0) / ranked.length
      : 0,
  };
}
